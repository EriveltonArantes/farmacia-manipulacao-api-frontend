...
const apiBaseUrl = "https://farmacia-manipulacao-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Farmaceutico</li>
          <li>✓ Controle completo de Formulacao</li>
          <li>✓ Controle completo de Orcamento Manipulacao</li>
          <li>✓ Controle completo de Ordem Manipulacao</li>
        </ul>
      </div>
...
function PainelFarmaceutico({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "crm", "email", "telefone", "especialidade", "ativo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/farmaceuticos", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/farmaceuticos/" + editId : apiBaseUrl + "/api/farmaceuticos";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "crm", "email", "telefone", "especialidade", "ativo"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/farmaceuticos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Farmaceutico")}</div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Farmaceutico</h3>
...
function PainelFormulacao({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "formaFarmaceutica", "descricao", "via", "validade", "ativa", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/formulacaos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/formulacaos/" + editId : apiBaseUrl + "/api/formulacaos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "formaFarmaceutica", "descricao", "via", "validade", "ativa"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/formulacaos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Formulacao")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>formaFarmaceutica</b><span>{item["formaFarmaceutica"] ?? "—"}</span></div>
            <div className="item-field"><b>descricao</b><span>{item["descricao"] ?? "—"}</span></div>
            <div className="item-field"><b>via</b><span>{item["via"] ?? "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Formulacao</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
      <label className="field-label">formaFarmaceutica</label>
      <input type="text" value={form["formaFarmaceutica"] ?? ""} onChange={e => setForm({...form, formaFarmaceutica: e.target.value})} />
      <label className="field-label">descricao</label>
      <textarea rows={3} value={form["descricao"] ?? ""} onChange={e => setForm({...form, descricao: e.target.value})} />
      <label className="field-label">via</label>
      <input type="text" value={form["via"] ?? ""} onChange={e => setForm({...form, via: e.target.value})} />
...
function PainelOrcamentoManipulacao({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [formulacaoList, setFormulacaoList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["clienteNome", "clienteCpf", "telefone", "formulacaoId", "quantidade", "valor", "prazoEntrega", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/orcamentomanipulacaos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/formulacaos", { headers: authHeaders(token) }).then(r => r.json()).then(setFormulacaoList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/orcamentomanipulacaos/" + editId : apiBaseUrl + "/api/orcamentomanipulacaos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["clienteNome", "clienteCpf", "telefone", "formulacaoId", "quantidade", "valor", "prazoEntrega", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["formulacao"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/orcamentomanipulacaos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["clienteNome"] ?? "OrcamentoManipulacao")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/orcamentomanipulacaos/" + item.id, {
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              <div className="item-photo-fallback">O</div>
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/orcamentomanipulacao" + item.id + "/320/200"} alt="OrcamentoManipulacao" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🗂️</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["clienteNome"] ?? "OrcamentoManipulacao")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>clienteCpf</b><span>{item["clienteCpf"] ?? "—"}</span></div>
            <div className="item-field"><b>telefone</b><span>{item["telefone"] ?? "—"}</span></div>
            <div className="item-field"><b>formulacaoId</b><span>{item["formulacaoId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>prazoEntrega</b><span>{item["prazoEntrega"] ? new Date(item["prazoEntrega"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>formulacao</b><span>{item.formulacaoId ? ((formulacaoList.find(o => o.id === item.formulacaoId) || {}).nome ?? ("#" + item.formulacaoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} OrcamentoManipulacao</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">clienteNome</label>
      <input type="text" value={form["clienteNome"] ?? ""} onChange={e => setForm({...form, clienteNome: e.target.value})} />
      <label className="field-label">clienteCpf</label>
      <input type="text" value={form["clienteCpf"] ?? ""} onChange={e => setForm({...form, clienteCpf: e.target.value})} />
      <label className="field-label">telefone</label>
      <input type="text" maxLength={15} value={form["telefone"] ?? ""} onChange={e => setForm({...form, telefone: formatarTelefone(e.target.value)})} />
      <label className="field-label">formulacaoId</label>
      <input type="number" value={form["formulacaoId"] ?? ""} onChange={e => setForm({...form, formulacaoId: e.target.value})} />
      ...
      <label className="field-label">prazoEntrega</label>
      <input type="datetime-local" value={form["prazoEntrega"] ?? ""} onChange={e => setForm({...form, prazoEntrega: e.target.value})} />
      ...
      <label className="field-label">formulacao</label>
      <select value={form["formulacao"] ?? ""} onChange={e => setForm({...form, formulacao: e.target.value})}>
        <option value="">Selecione...</option>
        {(formulacaoList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelOrdemManipulacao({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [pagina, setPagina] = React.useState(1);
  const [modoKanban, setModoKanban] = React.useState(false);
  const [orcamentoManipulacaoList, setOrcamentoManipulacaoList] = React.useState([]);
  const [farmaceuticoList, setFarmaceuticoList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["orcamentoManipulacaoId", "farmaceuticoId", "dataEntrada", "dataPrevista", "dataConclusao", "status", "lote", "observacoes", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/ordemmanipulacaos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/orcamentomanipulacaos", { headers: authHeaders(token) }).then(r => r.json()).then(setOrcamentoManipulacaoList).catch(() => {});
    fetch(apiBaseUrl + "/api/farmaceuticos", { headers: authHeaders(token) }).then(r => r.json()).then(setFarmaceuticoList).catch(() => {});
  }, [token]);
  ...
  const montarCorpo = () => {
    const corpo = { ...form };
    ["farmaceutico", "orcamentoManipulacao"].forEach(k => {
      corpo[k + "Id"] = corpo[k] ? Number(corpo[k]) : null;
      delete corpo[k];
    });
    // Bug real (2026-06-26): <input type="datetime-local"> envia "YYYY-MM-DDTHH:mm"
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/ordemmanipulacaos/" + editId : apiBaseUrl + "/api/ordemmanipulacaos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["orcamentoManipulacaoId", "farmaceuticoId", "dataEntrada", "dataPrevista", "dataConclusao", "status", "lote", "observacoes"].forEach(k => { f[k] = item[k] ?? ""; });
    ["farmaceutico", "orcamentoManipulacao"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/ordemmanipulacaos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["status"] ?? "OrdemManipulacao")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/ordemmanipulacaos/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["status"] ?? "OrdemManipulacao")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>orcamentoManipulacaoId</b><span>{item["orcamentoManipulacaoId"] ?? "—"}</span></div>
            <div className="item-field"><b>farmaceuticoId</b><span>{item["farmaceuticoId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>dataConclusao</b><span>{item["dataConclusao"] ? new Date(item["dataConclusao"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>lote</b><span>{item["lote"] ?? "—"}</span></div>
            <div className="item-field"><b>observacoes</b><span>{item["observacoes"] ?? "—"}</span></div>
            <div className="item-field"><b>farmaceutico</b><span>{item.farmaceuticoId ? ((farmaceuticoList.find(o => o.id === item.farmaceuticoId) || {}).nome ?? ("#" + item.farmaceuticoId)) : "—"}</span></div>
            <div className="item-field"><b>orcamentoManipulacao</b><span>{item.orcamentoManipulacaoId ? ((orcamentoManipulacaoList.find(o => o.id === item.orcamentoManipulacaoId) || {}).clienteNome ?? ("#" + item.orcamentoManipulacaoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} OrdemManipulacao</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">orcamentoManipulacaoId</label>
      <input type="number" value={form["orcamentoManipulacaoId"] ?? ""} onChange={e => setForm({...form, orcamentoManipulacaoId: e.target.value})} />
      <label className="field-label">farmaceuticoId</label>
      <input type="number" value={form["farmaceuticoId"] ?? ""} onChange={e => setForm({...form, farmaceuticoId: e.target.value})} />
      ...
      <label className="field-label">dataConclusao</label>
      <input type="datetime-local" value={form["dataConclusao"] ?? ""} onChange={e => setForm({...form, dataConclusao: e.target.value})} />
      <label className="field-label">status</label>
      ...
      <label className="field-label">farmaceutico</label>
      <select value={form["farmaceutico"] ?? ""} onChange={e => setForm({...form, farmaceutico: e.target.value})}>
        <option value="">Selecione...</option>
        {(farmaceuticoList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
      <label className="field-label">orcamentoManipulacao</label>
      <select value={form["orcamentoManipulacao"] ?? ""} onChange={e => setForm({...form, orcamentoManipulacao: e.target.value})}>
        <option value="">Selecione...</option>
        {(orcamentoManipulacaoList || []).map(o => (<option key={o.id} value={o.id}>{o.clienteNome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Farmaceutico");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Farmacia Manipulacao<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Farmaceutico" ? " active" : "")} onClick={() => setAba("Farmaceutico")}><span className="nav-ico">🗂️</span>Farmaceutico</button>
        <button className={"nav-btn" + (aba === "Formulacao" ? " active" : "")} onClick={() => setAba("Formulacao")}><span className="nav-ico">🗂️</span>Formulacao</button>
        <button className={"nav-btn" + (aba === "OrcamentoManipulacao" ? " active" : "")} onClick={() => setAba("OrcamentoManipulacao")}><span className="nav-ico">🗂️</span>OrcamentoManipulacao</button>
        <button className={"nav-btn" + (aba === "OrdemManipulacao" ? " active" : "")} onClick={() => setAba("OrdemManipulacao")}><span className="nav-ico">🧾</span>OrdemManipulacao</button>
      ...
      <main className="main">
        ...
        {aba === "Farmaceutico" && <PainelFarmaceutico token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Formulacao" && <PainelFormulacao token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "OrcamentoManipulacao" && <PainelOrcamentoManipulacao token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "OrdemManipulacao" && <PainelOrdemManipulacao token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
